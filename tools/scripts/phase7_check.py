...
required = [
    ...
    "examples/07-task-delay-timeout/README.md",
...
if built_target:
    elf = ROOT / "build/07-task-delay-timeout/hairtos_baremetal.elf"
    ...
    if nm_tool:
        ...
        systick_lines = [line for line in symbols.splitlines() if "SysTick_Handler" in line]
        if not systick_lines or not any(re.search(r"\bT\b", line) for line in systick_lines):
            print("SysTick_Handler is not a strong text symbol")
            sys.exit(1)
    ...
    if objdump_tool:
        ...
        for item in [
            ...
            if item not in lowered:
                print(f"Phase 7 disassembly missing: {item}")
                sys.exit(1)
    ...
    print("HairRTOS Phase 1/3/4/5/6 target regression builds: PASS")
    print("HairRTOS Phase 7 SysTick/delay target build: PASS")
    print("HairRTOS Phase 7 symbols/disassembly: PASS")

print("HairRTOS Phase 7 structure check: PASS")
print("HairRTOS Phase 7 host tests: PASS")
print("Physical STM32F103 delay/tick runtime validation: required on target hardware")
