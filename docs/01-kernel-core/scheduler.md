...
[← Root README](../../README.md) · [↑ Back to section](README.md) · [← Previous](memory-model.md) · [Next →](task-model.md)
...
- ISR wakeup.
- a READY task must be moved to its new ready queue;
- a blocked waiter may need to be reordered in the wait list;

Key implementation details:

- propagation may continue to an upstream owner.
- the bitmap reflects non-empty queues;
- the running task has the correct state/membership;
- idle is the fallback;
- a blocked task has no ready node;
- ready ordering uses effective priority.
