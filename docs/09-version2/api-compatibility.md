# API compatibility Version 2

> **Status: FUTURE DESIGN.** This content is not a capability of `hairtos 1.0.0-rc1`.

[← Root README](../../README.md) · [↑ Back to section](README.md) · [Next →](architecture.md)
...
- The v1 public headers are the baseline; internal layouts carry no compatibility guarantee.
- A breaking change must include a migration note, compile-time guard, or versioned contract.
- Do not publish headers ahead of implementation/tests merely to “reserve an API”.
- fix semantic defects;
- enable HSM/port contracts that cannot be cleanly added as extensions;
- remove unsafe contracts;
- substantially reduce coupling.
- 1.x: compatibility-focused stabilization;
- 2.0: controlled breaking changes;
- 2.x: backward-compatible features/fixes under the new public contract.
