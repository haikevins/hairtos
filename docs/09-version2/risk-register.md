...
- Scope creep across HSM/tickless/trace/second-target work can reduce auditability; phase gates are the primary mitigation.
- An incorrect interrupt ceiling can create deadlocks/races that are harder to diagnose than PRIMASK; define the contract and add negative tests.
- Trace overhead can perturb timing; it must be bounded and compile-time disableable.
- An overly broad portability abstraction can become a new HAL; target capabilities must keep the contract minimal.
- no clear use case;
- cannot be tested deterministically;
- introduces reverse dependencies;
- makes static memory consumption unpredictable;
- delays mandatory goals such as a second target, HSM support, and validation.
