...
- The success criterion is a second target that builds, runs examples, and passes hardware tests—not merely an abstraction that compiles.
- Target capabilities should describe FPU/MPU/interrupt masking/benchmark/tickless support rather than duplicating source lists.
- Application examples should be capability-gated instead of hard-coding Blue Pill assumptions.
- Cortex-M4/M4F MCU: reuse the ARM exception model but test FPU and priority differences;
- Cortex-M0+: requires a distinct port assembly implementation and capability testing.
