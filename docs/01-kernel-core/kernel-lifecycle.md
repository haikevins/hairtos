...
[← Root README](../../README.md) · [↑ Back to section](README.md) · [← Previous](kernel-invariants.md) · [Next →](memory-model.md)
...
- reset the timer system when the feature is linked;
- transition state to INITIALIZED.
- task count never exceeds `HR_CFG_MAX_TASKS`;
- current task is NULL before RUNNING;
- RUNNING has exactly one current task;
- the idle task is valid;
- kernel start cannot be called twice.
