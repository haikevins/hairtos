...
- the application knows the footprint at compile time;
- internal layout is not public;
- no heap is required;
- alignment is guaranteed through `max_align_t`.
- alignment 8 bytes;
- fill `0xA5A5A5A5`;
...
- the initial frame is constructed so exception return enters the task entry function.
- copy `.data`;
- zero `.bss`;
- the retained diagnostics section is not zero-initialized.
- no MPU guard region;
- stack overflow can corrupt memory before a health check detects it;
- opaque storage sizes include fixed headroom;
- no per-object memory-provenance tracking.
