...
- ordering.
- double insert;
- removing a node that is not linked;
- moving a node but forgetting to update the bitmap;

Key implementation details:

- effective priority changes but the wait node is not reinserted;
- timeout completes while the wait node remains linked.
